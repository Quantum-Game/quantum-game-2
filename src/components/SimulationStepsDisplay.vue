<template>
	<div class="simulation-steps-display-wrapper">
		<div v-for="(frame, index) in frames" :key="index" class="step">
			<h3>STEP {{ frame.step }}</h3>
			<div v-for="(particle, pindex) in frame.quantum" :key="`particle-${pindex}`" class="particle">
				<div>
					A:
