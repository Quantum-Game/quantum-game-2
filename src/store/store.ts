...
const store: StoreOptions<RootState> = {
  ...
  //       const cellToDeleteIndex = state.currentLevel.cells
  // .indexOf(getters.cell({ x, y, element }));
  //       dispatch('deleteCell', { originX, originY });
  //     }
  ...
  // },
  // getters: {
  //   cell: state => (y: number, x: number) => state.currentLevel.cells.
  // find((o: {x: number, y: number}) => o.x === x && o.y === y),
  //   tools: state => state.currentLevel.cells.filter(cell => cell.x === -1 && cell.y === -1),
  //   cellFromTray: state => (element: string) => state.currentLevel.cells
// .find(o => o.element === element),
  // },
  // modules: {
